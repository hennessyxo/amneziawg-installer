...
A native desktop app: a point-and-click front end over the same SSH logic the CLI
(`cmd/awg-deploy`) uses. Connect to a Linux server, install AmneziaWG, and manage
clients without touching a terminal.

Built with [Wails v2](https://wails.io) (Go backend with a tiny vanilla HTML/CSS/JS
...
2. **Install**: one tuned profile (mobile params: MTU 1280 + Jc=3, reliable on
   both 4G/LTE and PC); obfuscation profile, DNS and UDP port under *Advanced*.
   Streams progress live.
3. **Clients**: list, add (scannable QR + downloadable `.conf`), show any
   client's config/QR later, rename, remove. Per-client routes/DNS/MTU under
   *Advanced*.
4. **Monitoring**: live VPN status, uptime, version, per-client traffic with
   sortable columns, plus aggregate traffic over day/week/month.
...
6. **Telegram bot**: install or remove the access-controlled bot from the app
   (token, allowed IDs, access password), with an in-window setup guide.
7. **Settings**: server info (IP, UDP port, AmneziaWG version, uptime, client
   count), rename the connection, change the web-panel password, or remove the
   panel / AmneziaWG entirely (danger zone).
8. **Server terminal**: run commands on the server over SSH and see their output,
   with a list of handy diagnostics you can click.
...
- Host keys use **trust-on-first-use**: an unknown server key is recorded in
  ...
  has no terminal to confirm on, so first-use trust is automatic, the same
  behaviour as the CLI's `--accept-new`.

## Status

The Go backend, the JS-to-Go bridge and the cross-platform build are verified by
CI. Visual/interaction testing is manual (run `wails build` and open the app),
since a GUI cannot be exercised in headless CI.
