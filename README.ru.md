...
1. Скачай бинарник `awg-deploy` **под свой компьютер** из
   ...
   (Архивы `.tar.gz`/`.zip` сохраняют флаг «исполняемый» — `chmod` не нужен.
   Сырые бинарники тоже приложены, если так удобнее.)

2. **Просто запусти без аргументов** — спросит адрес сервера и пароль root,
   подключится по SSH и запустит установку и меню управления **прямо на сервере**:
   установка (если нужно), затем добавить / удалить / переименовать клиентов,
   мониторинг, веб-панель, удаление — **все действия идут на сервере**, ты только
   ...
   **macOS / Linux** — дважды кликни по `.tar.gz`, чтобы распаковать (или
   `tar -xzf awg-deploy-darwin-arm64.tar.gz`), затем:
   ```bash
   xattr -dr com.apple.quarantine ./awg-deploy   # только macOS: снять карантин Gatekeeper
   ./awg-deploy
   ```
