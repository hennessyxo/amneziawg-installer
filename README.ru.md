...
Скрипт задаст несколько вопросов (публичный IP, порт, DNS, имя первого клиента,
мобильный пресет) и покажет QR-код для импорта в приложение **AmneziaVPN**.
Запусти скрипт снова в любой момент — откроется меню управления (клиенты,
мониторинг, веб-панель).

### Установка из Windows / по SSH

Не хочешь заходить на сервер? Скачай `awg-deploy` из
[Releases](https://github.com/hennessyxo/amneziawg-installer/releases) — один
бинарник (`.exe` для Windows, плюс macOS/Linux):

```bash
awg-deploy install root@203.0.113.7 --preset mobile   # ставит по SSH, печатает QR
awg-deploy add-client root@203.0.113.7 laptop         # новый клиент + QR
awg-deploy monitor root@203.0.113.7                   # дашборд локально
```

Скрипт установщика встроен в бинарник — на сервер ничего качать не надо. См.
