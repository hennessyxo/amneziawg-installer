...
2. Запусти из терминала, указав свой сервер:

   **Windows** (PowerShell / Windows Terminal):
   ```powershell
   .\awg-deploy-windows-amd64.exe install root@IP_СЕРВЕРА --preset mobile
   ```
   ...
   **macOS / Linux:**
   ...
   ./awg-deploy-darwin-arm64 install root@IP_СЕРВЕРА --preset mobile
   ```

   Спросит SSH-пароль (или укажи ключ: `--identity ~/.ssh/id_ed25519`), поставит
   AmneziaWG по SSH, сохранит клиентский `.conf` и нарисует QR. Скрипт установщика
   **встроен в бинарник** — на сервер ничего качать не надо.
...
3. Управление так же:
   ```bash
   awg-deploy add-client    root@IP_СЕРВЕРА laptop   # новый клиент + QR
   awg-deploy list          root@IP_СЕРВЕРА          # список клиентов
   awg-deploy remove-client root@IP_СЕРВЕРА laptop   # удалить клиента
   awg-deploy menu          root@IP_СЕРВЕРА          # интерактивное меню по SSH
   awg-deploy monitor       root@IP_СЕРВЕРА          # живой дашборд
   awg-deploy uninstall     root@IP_СЕРВЕРА          # удалить всё (спросит подтверждение)
   ```
