...
var langs = map[string]map[string]string{
	"ru": {
		...
		"f_limits":      "Лимиты",
		"f_advanced":    "Расширенные",
		"f_routes":      "Маршруты (AllowedIPs)",
		"f_dns":         "DNS",
		"f_mtu":         "MTU",
		"hint_advanced": "пусто = весь трафик (0.0.0.0/0) и серверные DNS/MTU. Split-tunnel: укажи нужные подсети.",
		"tip_routes":    "Какой трафик клиент шлёт через VPN. Пусто = весь (0.0.0.0/0). Укажи подсети для split-tunnel — тогда через VPN идут только они, остальное напрямую.",
		"tip_dns":       "DNS-серверы для клиента. Пусто = серверные по умолчанию. Можно указать свои через запятую.",
		"tip_mtu":       "Размер пакета. Пусто = как у сервера. Меньше (напр. 1280) помогает на мобильных сетях.",
	...
	"en": {
		...
		"tip_dns":       "DNS servers the client uses. Empty = the server defaults. You can set your own, comma-separated.",
		"tip_mtu":       "Packet size. Empty = same as the server. Smaller (e.g. 1280) helps on mobile networks.",
		"optional":      "optional",
		"online":        "online",
		"stat_dn":       "↓ total",
		"stat_up":       "↑ total",
